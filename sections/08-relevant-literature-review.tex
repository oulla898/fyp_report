...
\hline
\end{tabular}
\caption{Competitive analysis of leading hospital AMR platforms by global deployment and capabilities \cite{pudubot2, m2medical, puducc1}.}
...
\hline
OpenAI GPT-5.2 & OpenAI & Proprietary & 128,000 & \$2.50 & \$10.00 \\
\hline
Google Gemini 3 flash & Google & Proprietary & 1,048,576 & \$1.25--\$2.50 & \$10.00--\$15.00 \\
\hline
...
\hline
\end{tabularx}
\caption{Comparative analysis of leading Large Language Models. Data compiled from early 2025 period \cite{civo2024, medgemini2024}.}
\label{tab:llm_comparison}
\end{table}

\paragraph{LLM Discussion and Cost Considerations.}
Table~\ref{tab:llm_comparison} summarizes key characteristics of leading LLM options. OpenAI's GPT-5.2 offers strong performance with a 128k token context window, while Google's Gemini 3 flash stands out with exceptional context length (over 1M tokens) and competitive API pricing. Notably, Google's Gemini API is available at no cost for requests under 60 requests per minute (RPM), making it an attractive option for development and testing phases. Google's Med-Gemma represents a family of specialized open-weight models (available in 2B, 7B, and 27B parameter variants) fine-tuned specifically on medical literature, clinical guidelines, and healthcare datasets \cite{medgemini2024}. While Med-Gemma demonstrates state-of-the-art performance on medical benchmarks (MedQA, MedMCQA, PubMedQA) and would be highly relevant for advanced clinical decision support or medical question answering, it is considered overkill for the basic conversational interactions required in this project (patient guidance, wayfinding, simple information queries). Meta's Llama 3.1 405B provides an open-source alternative with flexible licensing, while DeepSeek V3 offers the most economical API costs, particularly beneficial for cost-sensitive deployments. Even that Llama and DeepSeek are open-source models, they still have associated costs when using hosted APIs, which can vary based on usage volume and specific service providers.
