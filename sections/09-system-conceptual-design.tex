...
\begin{itemize}
    ...
    \item \textbf{Intelligence}: Custom integration of Google Gemini 3.0 pro via API for conversational AI, with fallback to on-device models
...
\textbf{LLM Integrated} & No (requires custom integration) & No (requires custom integration) & No (custom Gemini integration) \\
...
For the conversational AI component, three state-of-the-art LLM options are evaluated: OpenAI GPT-4 Turbo, Anthropic Claude 3.5 Sonnet, and Google Gemini 2.0 Flash (announced December 2024).
...
\midrule
GPT-4 Turbo & 7 & 9 & 6 & 5 \\
Claude 3.5 Sonnet & 5 & 9 & 7 & 6 \\
Gemini 3.0 pro & 9 & 8 & 9 & 9 \\
\bottomrule
\end{tabular}
\end{table}
...
Scoring rationale:
\begin{itemize}
    \item \textbf{GPT-4 Turbo}: Excellent performance, moderate familiarity, good but not fastest latency, moderate cost
    \item \textbf{Claude 3.5 Sonnet}: Excellent performance with strong safety features, less team familiarity, good latency, moderate cost
    \item \textbf{Gemini 3.0 pro}: High team familiarity with Google AI APIs, excellent performance for size, extremely low latency optimized for real-time use, very cost-effective
\end{itemize}
...
\begin{tabular}{lc}
\toprule
\textbf{LLM} & \textbf{Final Score} \\
\midrule
GPT-4 Turbo & 7.62 \\
Claude 3.5 Sonnet & 7.54 \\
\textbf{Gemini 3.0 pro} & \textbf{8.44} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Google Gemini 2.0 Flash} is selected based on its superior AHP score. Key advantages include:
\begin{itemize}
...
LLM & Google Gemini 2.0 Flash & 8.44 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{AI/LLM}: Google Gemini 3.0 pro via Cloud API
...
At the highest abstraction level, the smart healthcare robot can be represented as a single system with external interfaces:
...
The system is decomposed into six major subsystems, each with defined responsibilities:
...
\begin{itemize}
    \item \textbf{Components}: Gemini 3.0 pro API client, speech-to-text, text-to-speech, dialogue manager
...
This chapter presented a systematic approach to the conceptual design of the smart healthcare robot. Through AHP-based evaluation of three platform alternatives (commercial, custom-built, and hybrid), the hybrid approach was selected as it balances reliability with customizability for healthcare-specific AI features. A second AHP analysis selected Google Gemini 3.0 pro as the optimal LLM for conversational AI due to team familiarity, low latency, and cost-effectiveness.
