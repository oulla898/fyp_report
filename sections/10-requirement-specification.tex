...
\begin{itemize}
    \item \textbf{CR1: Autonomous Navigation and Mobility.} The robot must autonomously navigate hospital corridors, patient rooms, and common areas without human intervention, avoiding static and dynamic obstacles. Manual navigation of telepresence robots has been identified as a significant limitation, increasing cognitive load on clinicians and reducing system usability \cite{kristoffersson2013}. Autonomous navigation frees clinical staff from control tasks and enables efficient, reliable point-to-point movement.
    ...
    \item \textbf{CR3: Patient Engagement and Social Interaction.} The robot must engage with patients through natural language conversation, providing information, companionship, and emotional support, particularly for pediatric and elderly populations. Socially assistive robots have demonstrated significant benefits in reducing patient anxiety, improving treatment compliance, and enhancing overall patient experience \cite{beran2013, broekens2009}.
    ...
    \item \textbf{CR6: Continuous Operation and Autonomous Charging.} The robot must operate throughout the day with minimal downtime, autonomously returning to a charging dock when battery levels are low. Hospital operations are 24/7. The robot must support continuous availability without requiring manual intervention for battery management, as implemented in commercial platforms \cite{irobot2024create3, turtlebot2024}.
\end{itemize}
...
\textbf{Related Standards:} ISO 13482:2014 (Safety requirements for personal care robots - mobile servant robots).\\
...
\textbf{Mapped to:} CR1 (Autonomous Navigation)\\
\textbf{Related Standards:} ISO 13482:2014 (Speed limits for safe human-robot interaction).\\
...
\textbf{Mapped to:} CR1 (Autonomous Navigation)\\
\textbf{Related Standards:} ISO 13855 (Safety of machinery - Positioning of safeguards).\\
\textbf{Verification Method:} Obstacle avoidance tests with various object types and approach angles.

\textbf{ER1.4: Payload Capacity}\\
...
\textbf{Mapped to:} CR2 (Telepresence)\\
\textbf{Related Standards:} ITU-T H.264 / H.265 (Advanced Video Coding).\\
...
\textbf{Mapped to:} CR2 (Telepresence)\\
\textbf{Related Standards:} IEEE 802.11 (Wi-Fi Standards).\\
...
\textbf{Mapped to:} CR5 (User Interface)\\
...
\textbf{Mapped to:} CR6 (Continuous Operation)\\
...
\textbf{Related Standards:} ISO 13850 (Safety of machinery - Emergency stop function).\\
...
This section analyzes the performance data presented in Section 3.3.6 to derive optimal system configurations. The discussion focuses on interpreting the trade-offs between performance, power, and cost to satisfy the engineering requirements and constraints.
...
\end{itemize}
\textbf{Observation:} A hybrid approach combining 2D LiDAR for long-range SLAM and RGB-D for near-field safety offers the best balance, maximizing coverage while keeping total sensor cost under \$400.
...
\end{itemize}
\textbf{Observation:} The Raspberry Pi 5 is the most balanced choice for the core navigation controller. AI tasks can be offloaded to the cloud (via API) to avoid the need for expensive onboard GPU hardware, aligning with EC1 and EC4.
...
\end{itemize}
\textbf{Observation:} Cloud-based inference using Gemini 2.5 Pro is the optimal strategy, satisfying performance requirements (ER3.1, ER3.2) while eliminating operational costs during the development phase.

\subsection{Conclusion}

This chapter established the requirement specification for the smart healthcare robot, defining clear customer needs, engineering specifications, and system constraints. The comparative performance analysis in Section 3.3.6 provided empirical data on component trade-offs, highlighting the tension between performance, power, and cost.

The subsequent analysis in Section 3.5 demonstrated that no single component satisfies all requirements perfectly. Instead, a system-level optimization approach is required:
\begin{itemize}
